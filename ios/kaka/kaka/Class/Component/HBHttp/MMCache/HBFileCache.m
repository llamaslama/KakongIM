...
+(HBFileCache*)shareCache
{
    ...
    dispatch_once(&onceToken, ^{
        ...
        [[MMDBHelper shareDBHelper] createTableWithModelClass:[HBFileCacheData class]];
    });
    return cache;
}
...
-(NSString*)storeFile:(NSData*)fileData
{
    ...
    cache.cacheId=[[MMDBHelper shareDBHelper] newIdforTable:[HBFileCacheData class]];
    ...
    if([[MMDBHelper shareDBHelper] insertToDB:cache]){
        if (![fileData writeToFile:cache.cacheFileName atomically:NO]) {
            [[MMDBHelper shareDBHelper] deleteToDB:cache];
            return  nil;
        }else{
...
-(BOOL)storeFile:(NSData *)fileData forUrl:(NSString*)url
{
    if([NSStrUtil isEmptyOrNull:url])
        return NO;
    int rowCount=[[MMDBHelper shareDBHelper] rowCount:[HBFileCacheData class] where:@{@"filekey":[url  md5]}];
    if(rowCount==0){
        HBFileCacheData * cache=[[HBFileCacheData alloc]init];
        cache.cacheId=[[MMDBHelper shareDBHelper] newIdforTable:[HBFileCacheData class]];
        ...
        HBFileCacheData * cache=[[MMDBHelper shareDBHelper] searchSingle:[HBFileCacheData class] where:@{@"filekey":[url  md5]} orderBy:nil];
        if (![fileData writeToFile:cache.cacheFileName atomically:NO]) {
            [[MMDBHelper shareDBHelper] deleteToDB:cache];
            return  NO;
        }else{
...
-(NSData*)getDataFromDisk:(NSString *)key
{
    if([NSStrUtil notEmptyOrNull:key]){
        HBFileCacheData * cache=[[MMDBHelper shareDBHelper] searchSingle:[HBFileCacheData class] where:@{@"fileKey":[key md5]} orderBy:nil];
        if(cache){
...
{
    HBFileCacheData * cache=[[MMDBHelper shareDBHelper] searchSingle:[HBFileCacheData class] where:@{@"filekey":[fromKey  md5]} orderBy:nil];
    if (cache) {
        NSFileManager * manager=[NSFileManager defaultManager];
        if ([manager fileExistsAtPath:cache.cacheFileName isDirectory:nil]) {
            HBFileCacheData * data=[[HBFileCacheData alloc]init];
            data.cacheId=[[MMDBHelper shareDBHelper] newIdforTable:[HBFileCacheData class]];
            data.filekey=[toKey md5];
            data.cacheFileName=cache.cacheFileName;
            [[MMDBHelper shareDBHelper]insertToDB:data];
        }
        [[MMDBHelper shareDBHelper] deleteToDB:cache];
    }
}
