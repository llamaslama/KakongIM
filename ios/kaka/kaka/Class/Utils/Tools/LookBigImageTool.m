...
#import "MMLookImageViewController.h"
...
-(void)lookBigImage:(UIImageView*)imageView smallImageViews:(NSArray*)smallImageViews bigImageUrls:(NSArray*)bigImageUrls viewController:(UIViewController*)viewController
{
    ...
    if (image) {
        toFrame=[MMImageUtil frameForImageView:image.size inRect:_backView.bounds];
    }else{
        toFrame=[MMImageUtil frameForImageView:imageView.image.size inRect:_backView.bounds];
    }
    [UIView animateWithDuration:0.3 delay:0 options:UIViewAnimationOptionCurveEaseOut animations:^{
        ...
        MMLookImageViewController * controller=(MMLookImageViewController *)[viewController viewControllerForName:@"MMLookImageViewController" Class:[MMLookImageViewController class]];
